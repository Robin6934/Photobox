...
export class Client extends BaseClass {
    ...
    postApiLogout(): Promise<void> {
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processPostApiLogout(_response);
        });
    }

    protected processPostApiLogout(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class PhotoBoxClient extends BaseClass {
     ...
     */
    getMyPhotoBoxes(): Promise<PhotoBoxResponse[]> {
        let url_ = this.baseUrl + "/api/PhotoBox/GetMyPhotoBoxes";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processGetMyPhotoBoxes(_response);
        });
    }

    protected processGetMyPhotoBoxes(response: Response): Promise<PhotoBoxResponse[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            ...
            if (Array.isArray(resultData200)) {
                result200 = [] as any;
                for (let item of resultData200)
                    result200!.push(PhotoBoxResponse.fromJS(item));
            }
            else {
                result200 = null as any;
            }
        ...
        return Promise.resolve<PhotoBoxResponse[]>(null as any);
    }
...
export class PhotoBoxResponse implements IPhotoBoxResponse {
    id?: string;
    name?: string;
    hardwareId?: string;
    ...
    constructor(data?: IPhotoBoxResponse) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (this as any)[property] = (data as any)[property];
    ...
    init(_data?: any) {
        if (_data) {
            ...
            this.name = _data["name"];
            this.hardwareId = _data["hardwareId"];
            this.currentEventCode = _data["currentEventCode"];
            this.currentEventName = _data["currentEventName"];
        }
    }

    static fromJS(data: any): PhotoBoxResponse {
        data = typeof data === 'object' ? data : {};
        let result = new PhotoBoxResponse();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        ...
        data["name"] = this.name;
        data["hardwareId"] = this.hardwareId;
        data["currentEventCode"] = this.currentEventCode;
        data["currentEventName"] = this.currentEventName;
        return data;
    }
}

export interface IPhotoBoxResponse {
    id?: string;
    name?: string;
    hardwareId?: string;
    currentEventCode?: string | undefined;
